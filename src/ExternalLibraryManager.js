...
export function getSymbol(scope, expression) {
	...
	const leafExists =
		scope && Object.prototype.hasOwnProperty.call(scope, path[0]);
	...
	if (!scope || path.length > 1 || !leafExists) {
		...
		logger.warn(
			`"${expression}" did not evaluate to a value. Last property tried: ${prop}`
		);
		return false;
	}
		...
		if (!injected[shouldDefineSymbol]) {
			injected[shouldDefineSymbol] = new Promise((fulfill, reject) => {
				let script = createElement(document, 'script', {
					...
					onload: () => {
						const symbolDescriptor = getSymbol(
							global,
							shouldDefineSymbol
						);
						...
						if (shouldDefineSymbol && !symbolDescriptor) {
							return reject(
								'Loaded, but expected interface was not found: '.concat(
									shouldDefineSymbol
								)
							);
